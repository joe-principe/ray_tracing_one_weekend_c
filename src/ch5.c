...
{
    float t;
    vec3 unit_dir, first, second, center, N, point_vec, sub_vec;
    ...
    if (t > 0.0) {
        point_at_parameter(r, t, &point_vec);
        subtract_vec(&sub_vec, &point_vec, &center);
        turn_into_unit_vector(&N, &sub_vec);

        set_elems(vec, get_x(&N) + 1, get_y(&N) + 1, get_z(&N) + 1);
        multiply_scalar(vec, 0.5f);
...
main(void)
{
    ...
    if (!output_file) {
        perror("Could not open ch5.ppm. Aborting.\n");
        exit(EXIT_FAILURE);
    } /* if */
